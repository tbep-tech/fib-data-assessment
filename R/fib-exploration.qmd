...
short_names <- tibble(OrgName = sort(unique(fib_tb$OrgName)),
                      shortName = c("Charlotte Cty Dept of Health",
                                    ...
                                    "Div of Env Health, Bureau of Water Prgms",
                                    "Env Protection Comm of Hillsborough Cty",
                                    "FLDEP, SW District",
                                    "FLDEP",
                                    "FLDEP",
                                    "Manatee County Parks & Nat Res Dept",
                                    "Pasco County Stormwater Management Div",
                                    "Pinellas Cty Dept of Env Management",
                                    "Polk County Nat Res Div",
                                    "SMR Communities Inc",
                                    "SWFWMD"))
...
fib_monthly_bayseg[3:5][is.na(fib_monthly_bayseg[3:5])] <- 0
...
fib_monthly_provider[3:5][is.na(fib_monthly_provider[3:5])] <- 0
...
fib_bayseg_long <- fib_monthly_bayseg |> 
    pivot_longer(3:5, names_to = "Indicator",
                 values_to = "Count")
```
...
fib_bayseg_long2 |> 
    ggplot() +
    geom_tile(aes(x = Date, y = Bay.Segment, fill = Count),
              na.rm = FALSE) +
...
fib_provider_long2 |> 
    ggplot() +
    geom_tile(aes(x = Date, y = OrgName, fill = Count),
              na.rm = FALSE) +
    scale_fill_viridis_c(direction = -1,
                         na.value = "gray") +
    facet_wrap(~Indicator, ncol = 3) +
